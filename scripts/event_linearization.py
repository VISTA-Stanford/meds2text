...
def main():
    ...
    try:
        ...
        print(
            f"Found {total_events} non-note events across {total_timestamps} timestamps in {args.xml_file}"
        )
        print("=" * 80)
        ...
        if total_timestamps > args.limit:
            ...
            remaining_events = sum(
                len(events_by_timestamp[ts]) for ts in sorted_timestamps[args.limit :]
            )
            print(
                f"... and {remaining_events} more events across {remaining_timestamps} more timestamps"
            )
